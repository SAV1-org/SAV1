...
template <typename T>
std::ostream &
PrintUnknownEnumValue(std::ostream &os, T value)
{
...
std::ostream &
operator<<(std::ostream &os, Lacing value)
{
    switch (value) {
        case Lacing::kNone:
            return os << "0 (none)";
...
std::ostream &
operator<<(std::ostream &os, MatrixCoefficients value)
{
    switch (value) {
...
std::ostream &
operator<<(std::ostream &os, Range value)
{
    switch (value) {
...
std::ostream &
operator<<(std::ostream &os, TransferCharacteristics value)
{
    switch (value) {
...
std::ostream &
operator<<(std::ostream &os, Primaries value)
{
    switch (value) {
...
std::ostream &
operator<<(std::ostream &os, ProjectionType value)
{
    switch (value) {
...
std::ostream &
operator<<(std::ostream &os, FlagInterlaced value)
{
    switch (value) {
...
std::ostream &
operator<<(std::ostream &os, StereoMode value)
{
    switch (value) {
...
std::ostream &
operator<<(std::ostream &os, DisplayUnit value)
{
    switch (value) {
...
std::ostream &
operator<<(std::ostream &os, AspectRatioType value)
{
    switch (value) {
...
std::ostream &
operator<<(std::ostream &os, AesSettingsCipherMode value)
{
    switch (value) {
...
std::ostream &
operator<<(std::ostream &os, ContentEncAlgo value)
{
    switch (value) {
...
std::ostream &
operator<<(std::ostream &os, ContentEncodingType value)
{
    switch (value) {
...
std::ostream &
operator<<(std::ostream &os, TrackType value)
{
    switch (value) {
...
std::ostream &
operator<<(std::ostream &os, const std::vector<std::uint8_t> &value)
{
...
class DemoCallback : public Callback {
   public:
    ...
    void
    PrintElementMetadata(const std::string &name,
                         const ElementMetadata &metadata)
    {
        // Since we aren't doing any seeking in this demo, we don't have to
        // worry about kUnknownHeaderSize or kUnknownElementPosition when
        // adding the position and sizes.
        ...
        // The ContentEncAESSettings element has the longest name (out of all
        // other master elements) at 21 characters. It's also the deepest
        // master element at a level of 6. Insert enough whitespace so there's
        // room for it.
    ...
    template <typename T>
    void
    PrintMandatoryElement(const std::string &name, const Element<T> &element)
    {
        std::cout << std::string(indent * spaces_per_indent, ' ') << name;
    ...
    template <typename T>
    void
    PrintMandatoryElement(const std::string &name,
    ...
    template <typename T>
    void
    PrintOptionalElement(const std::string &name, const Element<T> &element)
    {
        if (element.is_present()) {
            std::cout << std::string(indent * spaces_per_indent, ' ') << name
                      << ": " << element.value() << '\n';
    ...
    template <typename T>
    void
    PrintOptionalElement(const std::string &name,
                         const std::vector<Element<T>> &elements)
    {
        for (const Element<T> &element : elements) {
    ...
    void
    PrintMasterElement(const BlockAdditions &block_additions)
    {
        PrintMasterElement("BlockMore", block_additions.block_mores);
    }

    void
    PrintMasterElement(const BlockMore &block_more)
    {
    ...
    void
    PrintMasterElement(const Slices &slices)
    {
        PrintMasterElement("TimeSlice", slices.slices);
    }

    void
    PrintMasterElement(const TimeSlice &time_slice)
    {
        PrintOptionalElement("LaceNumber", time_slice.lace_number);
    }

    void
    PrintMasterElement(const Video &video)
    {
    ...
    void
    PrintMasterElement(const Colour &colour)
    {
        ...
        PrintOptionalElement("ChromaSubsamplingHorz",
                             colour.chroma_subsampling_x);
        PrintOptionalElement("ChromaSubsamplingVert",
    ...
    void
    PrintMasterElement(const MasteringMetadata &mastering_metadata)
    {
    ...
    void
    PrintMasterElement(const Projection &projection)
    {
        PrintMandatoryElement("ProjectionType", projection.type);
        PrintOptionalElement("ProjectionPrivate",
    ...
    void
    PrintMasterElement(const Audio &audio)
    {
        PrintMandatoryElement("SamplingFrequency", audio.sampling_frequency);
        PrintOptionalElement("OutputSamplingFrequency",
    ...
    void
    PrintMasterElement(const ContentEncodings &content_encodings)
    {
        PrintMasterElement("ContentEncoding", content_encodings.encodings);
    }

    void
    PrintMasterElement(const ContentEncoding &content_encoding)
    {
    ...
    void
    PrintMasterElement(const ContentEncryption &content_encryption)
    {
    ...
    void
    PrintMasterElement(const ContentEncAesSettings &content_enc_aes_settings)
    {
    ...
    void
    PrintMasterElement(const CueTrackPositions &cue_track_positions)
    {
        ...
        PrintOptionalElement("CueBlockNumber",
                             cue_track_positions.block_number);
    }

    void
    PrintMasterElement(const ChapterAtom &chapter_atom)
    {
    ...
    void
    PrintMasterElement(const ChapterDisplay &chapter_display)
    {
    ...
    void
    PrintMasterElement(const Targets &targets)
    {
    ...
    void
    PrintMasterElement(const SimpleTag &simple_tag)
    {
    ...
    // Element<> wrapper and print the underlying master element if it's
    // present.
    template <typename T>
    void
    PrintMasterElement(const std::string &name, const Element<T> &element)
    {
        if (element.is_present()) {
            std::cout << std::string(indent * spaces_per_indent, ' ') << name
                      << "\n";
    ...
    template <typename T>
    void
    PrintMasterElement(const std::string &name,
                       const std::vector<Element<T>> &elements)
    {
        for (const Element<T> &element : elements) {
    ...
    Status
    OnElementBegin(const ElementMetadata &metadata, Action *action) override
    {
        // Print out metadata for some level 1 elements that don't have
        // explicit callbacks.
        switch (metadata.id) {
            case Id::kSeekHead:
                indent = 1;
    ...
    Status
    OnUnknownElement(const ElementMetadata &metadata, Reader *reader,
    ...
    Status
    OnEbml(const ElementMetadata &metadata, const Ebml &ebml) override
    {
        indent = 0;
        ...
        PrintMandatoryElement("DocTypeReadVersion",
                              ebml.doc_type_read_version);
        return Status(Status::kOkCompleted);
    }

    Status
    OnVoid(const ElementMetadata &metadata, Reader *reader,
           std::uint64_t *bytes_remaining) override
    {
        // Output Void elements without any indentation because we aren't
        // tracking which element contains them.
        int original_indent = indent;
        indent = 0;
    ...
    Status
    OnSegmentBegin(const ElementMetadata &metadata, Action *action) override
    {
        indent = 0;
    ...
    Status
    OnSeek(const ElementMetadata &metadata, const Seek &seek) override
    {
        indent = 2;
                   ...
                   Action *action) override
    {
        ...
        // A properly muxed file will have Timecode and PrevSize first before
        // any SimpleBlock or BlockGroups. The parser takes advantage of this
        // and delays calling OnClusterBegin() until it hits the first
        // SimpleBlock or BlockGroup child (or the Cluster ends if it's empty).
        // It's possible for an improperly muxed file to have Timecode or
        // PrevSize after the first block, in which case they'll be absent here
        // and may be accessed in OnClusterEnd() when the Cluster and all its
        // children have been fully parsed. In this demo we assume the file has
        // been properly muxed.
        indent = 2;
                       ...
                       Action *action) override
    {
        ...
        std::string flags =
            (simple_block.is_visible) ? "visible" : "invisible";
    ...
    Status
    OnSimpleBlockEnd(const ElementMetadata & /* metadata */,
    ...
    Status
    OnBlockGroupBegin(const ElementMetadata &metadata, Action *action) override
    {
        indent = 2;
    ...
    Status
    OnBlockEnd(const ElementMetadata & /* metadata */,
    ...
    Status
    OnBlockGroupEnd(const ElementMetadata & /* metadata */,
    ...
    Status
    OnFrame(const FrameMetadata &metadata, Reader *reader,
            std::uint64_t *bytes_remaining) override
    {
    ...
    Status
    OnClusterEnd(const ElementMetadata & /* metadata */,
                 const Cluster & /* cluster */) override
    {
        // The Cluster and all its children have been fully parsed at this
        // point. If the file wasn't properly muxed and Timecode or PrevSize
        // were missing in OnClusterBegin(), they'll be set here (if the
        // Cluster contained them). In this demo we already handled them,
        // though.
        return Status(Status::kOkCompleted);
    }
    ...
    Status
    OnCuePoint(const ElementMetadata &metadata,
    ...
    Status
    OnEditionEntry(const ElementMetadata &metadata,
    ...
    Status
    OnTag(const ElementMetadata &metadata, const Tag &tag) override
    {
        indent = 2;
    ...
    Status
    OnSegmentEnd(const ElementMetadata & /* metadata */) override
    {
        return Status(Status::kOkCompleted);
    }
};

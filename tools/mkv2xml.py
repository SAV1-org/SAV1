...
if sys.version < '3':
    reload(sys)
    # hack to stop failing on Unicodish subtitles
    sys.setdefaultencoding('utf-8')
    range = xrange
    def maybe_decode(x): return x
else:
    def maybe_decode(x): return x.decode("ascii")
...
class MatroskaToText(mkvparse.MatroskaHandler):
    ...
    def format_block(self, indent_):
        ...
        if self.lb_keyframe:
            newdata += "\n  "+indent_+"<keyframe/>"
        if self.lb_invisible:
            newdata += "\n  "+indent_+"<invisible/>"
        if self.lb_discardable:
            newdata += "\n  "+indent_+"<discardable/>"
        for data_2 in self.lb_data:
            ...
            if self.lb_track_id in self.text_tracks:
                try:
                    ...
                    newdata += "\n  "+indent_ + \
                        "<data encoding=\"text\"><![CDATA["
                    newdata += data_2.replace("\x00",
                                              "").replace("]]>", "]]]]><![CDATA[>")
                    newdata += "]]></data>"
                    decoded_as_text = True
                except UnicodeDecodeError:
                    sys.stderr.write(
                        "UTF-8 decoding error, leaving as binary\n")
    ...
    def printtree(self, list_, indent):
        ...
        for (name_, (type_, data_)) in list_:
            ...
            if type_ == mkvparse.EbmlElementType.MASTER:
                ...
                print("%s<%s>%s</%s>" %
                      (indent_, opening_tag_content, data_, name_))
...
if len(sys.argv) > 1:
    ...
    if sys.argv[1] == "-C":
        nocluster_mode = True
        banlist = ["SeekHead", "CRC-32", "Void", "Cues", "PrevSize", "Position", "Cluster",
                   "Timecode", "Timestamp", "PrevSize", "EncryptedBlock", "BlockGroup", "SimpleBlock"]
...
mkvparse.mkvparse(sys.stdin, MatroskaToText(
    frozenset(banlist), nocluster_mode))

...
def read_simple_element(f, type_, size):
    ...
    if type_ == EET.UNSIGNED:
        ...
        data = read_fixedlength_number(f, size, True)
        data *= 1e-9
        data += (datetime.datetime(2001, 1, 1) -
        ...
        if size == 4:
            ...
            data = read_fixedlength_number(f, size, False)
            sys.stderr.write(
                "mkvparse: Floating point of size %d is not supported\n" % size)
            data = None
    else:
        data = f.read(size)
    return data
...
def handle_block(buffer, handler, cluster_timecode, timecode_scale=1000000, duration=None, header_removal_headers_for_tracks={}):
    ...
    if laceflags == 0x00:  # no lacing
        buf = buffer[pos:]
        handler.frame(tracknum, block_timecode, header_removal_prefix +
                      buf, 0, duration, f_keyframe, f_invisible, f_discardable)
        return
    ...
    for i in lengths:
        ...
        pos += i
        handler.frame(tracknum, block_timecode, header_removal_prefix+buf,
                      more_laced_frames, duration, f_keyframe, f_invisible, f_discardable)
        more_laced_frames -= 1
...
def mkvparse(f, handler):
    ...
    while f:
        ...
        try:
            if not resync_element_id:
                ...
                if not (id_ in element_types_names):
                    sys.stderr.write(
                        "mkvparse: Unknown element with id %x and size %d\n" % (id_, size))
                    (resync_element_id, resync_element_size,
                     resync_element_headersize) = resync(f)
            ...
            (type_, name) = element_types_names[id_]
            (type_, name) = element_types_names[id_]
            type_ = handler.begin_handling_ebml_element(
                id_, name, type_, hsize, size)
        ...
        if name == "EBML" and type(data) == list:
            ...
            if 'EBMLReadVersion' in d:
                if d['EBMLReadVersion'][1] > 1:
                    sys.stderr.write(
                        "mkvparse: Warning: EBMLReadVersion too big\n")
            if 'DocTypeReadVersion' in d:
                if d['DocTypeReadVersion'][1] > 2:
                    sys.stderr.write(
                        "mkvparse: Warning: DocTypeReadVersion too big\n")
            dt = d['DocType'][1]
            if dt != "matroska" and dt != "webm":
                sys.stderr.write(
                    "mkvparse: Warning: EBML DocType is not \"matroska\" or \"webm\"")
            ...
            for (ten, (_t, track)) in tree:
                ...
                if 'TrackTimestampScale' in d:
                    sys.stderr.write(
                        "mkvparse: Warning: TrackTimestampScale is not supported\n")
                if 'ContentEncodings' in d:
                    try:
                        compr = dict(d["ContentEncodings"]
                                     [1][0][1][1][0][1][1])
        ...
        # cluster contents:
            ...
            data = f.read(size)
            handle_block(data, handler, current_cluster_timecode,
                         timecode_scale, None,  header_removal_headers_for_tracks)
            ...
            if 'Block' in d2:
                handle_block(d2['Block'][1], handler, current_cluster_timecode,
                             timecode_scale, duration, header_removal_headers_for_tracks)
        else:
